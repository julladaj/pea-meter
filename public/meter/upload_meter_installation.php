...
if (isset($_FILES['file'])) {
    ...
    $target_path = DIR_UPLOAD . $id . "_meter." . $image_extension;
    $url_name = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]/upload/" . DIR_NAME . "/$id"."_meter.jpg?t=" . time();
    ...
    if ($result) {
        if ($image_extension === 'png') {
            ...
            $target_path = DIR_UPLOAD . $id . "_meter.jpg";
            imagejpeg($bg, $target_path, $quality);
            imagedestroy($bg);
        }

        $message = "หมายเลขคำร้อง *$id* ได้ทำการแนบรูปการติดตั้งมิเตอร์\nสามารถตรวจสอบเอกสารได้ที่\n$url_name";
        $token = LINE_TOKEN;
